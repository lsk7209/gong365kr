...
import { ArrowRight, CalendarClock, Search, ShieldCheck, TrendingUp } from "lucide-react";
...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "창업머니맵 - 지원사업 공고를 한 번에 확인",
    description: "창업지원사업, 정책자금, 지역별 공고와 마감 임박 정보를 한 화면에서 확인합니다.",
    type: "website",
    locale: "ko_KR"
  }
};
  ...
  return (
    <main className="min-h-screen bg-white">
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 md:grid-cols-[1.05fr_0.95fr] md:py-20">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300">
              진행 중인 공고를 먼저 보여주고, 마감된 공고도 삭제하지 않아 이전 모집 조건과 다음 모집 가능성을 함께 확인할 수 있습니다.
            </p>
          ...
          <aside className="rounded-3xl bg-[#16181c] p-6">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">지역·분야별 공고 집계</p>
              </div>
              <span className="rounded-full bg-brand px-3 py-1 text-xs font-semibold text-white">Live</span>
            </div>
            <div className="mt-5 grid gap-3">
              <MetricRow label="지역 기준 공고" value={`${totalPrograms}개`} />
              {categoryRows.length > 0 ? (
                categoryRows.map((item) => <MetricRow key={item.label} label={item.label} value={`${item.count}개`} />)
              ) : (
                <div className="rounded-2xl bg-white/5 p-4 text-sm text-slate-300">카테고리 데이터 동기화 대기 중</div>
              )}
            </div>
          </aside>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-4 px-4 py-10 md:grid-cols-3">
        <FeatureCard title="진행 중 공고 우선" description="접수 가능한 공고를 먼저 정렬하고, 마감된 공고는 하단에 기록으로 유지합니다." />
        <FeatureCard title="지역별 탐색" description="전국 지역 키워드를 기준으로 공고를 묶어 관심 지역만 빠르게 확인할 수 있습니다." />
        <FeatureCard title="신청 전 점검" description="자격, 서류, 접수 기간을 원문 공고 기준으로 확인하도록 동선을 연결했습니다." />
      </section>
      ...
      <section className="bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-10">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-normal tracking-tight text-ink">마감 임박 지원사업</h2>
              <p className="mt-1 text-sm text-slate-600">오늘 기준 신청 가능한 공고를 마감일 가까운 순서로 보여줍니다.</p>
            </div>
            <Link href={latestDeadlineUrl} className="inline-flex items-center gap-1 text-sm font-semibold text-brand">
              전체 보기
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-normal tracking-tight text-ink">다가오는 행사</h2>
            <p className="mt-1 text-sm text-slate-600">교육, 설명회, 전시 일정을 일정순으로 확인하세요.</p>
          </div>
        ...
        {upcomingEvents.length > 0 ? (
          ...
          <EmptyState title="등록된 행사 데이터 없음" description="공개 행사 항목이 동기화되면 자동으로 반영됩니다." />
        )}
      </section>
      ...
      <section className="border-y border-line bg-white">
        <div className="mx-auto max-w-6xl px-4 py-10">
          <div className="mb-5 grid gap-2 sm:grid-cols-2">
            <h2 className="text-2xl font-normal tracking-tight text-ink">지역별 지원사업 수</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {regionRows.map((region) => (
              ...
              >
                ...
                <strong className="mt-1 block text-lg font-semibold text-ink">{region.name}</strong>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <div className="rounded-3xl bg-[#0a0b0d] p-8 text-white md:p-12">
          ...
          <h2 className="mt-4 max-w-2xl text-3xl font-normal tracking-tight">마감된 공고도 검색 노출을 위해 유지합니다</h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300">
            신청이 끝난 공고는 삭제하지 않고 `마감` 상태로 표시합니다. 이전 모집 조건을 참고할 수 있고, 검색엔진에는 URL과 문맥이 유지됩니다.
          </p>
        </div>
      </section>
    </main>
